...
__global__ void MatrixFastPartialTraceKernel(const int m, const int size, REAL *A, REAL *result, int offset, int *jjp, int ncorerows)
{
    ...
    while (i < size) {
///*
      for (int jj = 0; jj < ncorerows; jj++)
      {
        j = jjp[jj]*m+i+offset;
        sdata[threadIdx.x] += A[j] * A[j];
      }
//*/
/*
      j = jjp[i]*m+offset;
      for (int k = 0; k < m; k++)
      {
        //sdata[threadIdx.x] += A[i+offset] * A[i+offset];
        sdata[threadIdx.x] += A[j+k] * A[j+k];
      }
*/
