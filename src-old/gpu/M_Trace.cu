...
REAL M_PartialTraceAsync(Matrix A, int *device_jjp, int ncorerows, int idevice, int istream) {
  ...
  MatrixFastPartialTraceKernel<<<blockCount,NUM_THREADS,smemSize,stream[idevice][istream]>>>(A.DM, sub, A.Device[0], device_trace, 0, device_jjp, ncorerows);
...
REAL M_PartialTraceMgpuStrm(Matrix A, Vector jjp, int ncorerows, int istream) {
  ...
  // Size is N * block size
  //int sub = (ncorerows * A.DN) / nblocks;
  //int bsize = (A.DN * ncorerows) * sub;

  //int sub = ncorerows / nblocks;
  //int bsize = A.DN * sub;
...
*/
  ...
  for (int d = 0; d < ndevices; ++d) {
    ...
    for (int b = d; b < nblocks; b+=ndevices) {
      ...
      MatrixFastPartialTraceKernel<<<blockCount,NUM_THREADS,smemSize,stream[d][istream]>>>(A.DM, sub, A.Device[d]+b*bsize, device_trace, b*sub, jjp.Device[d], ncorerows);

      // Copy to local variable
      M_WaitSingleStrm(d, istream);
      cudaMemcpy(local_trace, device_trace, blockCount * sizeof(REAL), cudaMemcpyDeviceToHost);
