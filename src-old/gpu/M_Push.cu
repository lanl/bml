...
void M_PushAsync(Matrix A, int idevice, int istream) {
  ...
  status=cublasSetMatrixAsync(A.M, A.N, sizeof(REAL), A.Local, A.M, A.Device[0], A.DM, stream[idevice][istream]);
  if (status!=CUBLAS_STATUS_SUCCESS) {
    if (status==CUBLAS_STATUS_NOT_INITIALIZED) printf("PushAsync: CuBLAS not initialized!\n");
    if (status==CUBLAS_STATUS_INVALID_VALUE) printf("PushAsync: Invalid value!\n");
    if (status==CUBLAS_STATUS_MAPPING_ERROR) printf("PushAsync: error accessing GPU memory!\n");
    exit(1);
  }
...
void M_PushVectorAsync(Vector A, int idevice, int istream) {
  ...
  cudaSetDevice(0);
  cudaMemcpyAsync(A.Device[0], A.Local, A.DN * sizeof(int), cudaMemcpyHostToDevice, stream[idevice][istream]);
