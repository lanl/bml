...
contains
  ...
  !!
  !! A note on the choice of allocatable
  !!
  !! Some dummy matrix arguments here (the ones that are going to
  !! allocated something) need the allocatable attribute. Once the
  !! dynamic type is narrowed down to e.g. dense, the next level
  !! procedures can not have this attribute anymore as Fortran does
  !! not allow that.
  !!
  ...
  subroutine bml_deallocate(a)
    ...
    ! The allocated() test has to be done here and not one layer down
    ! (for a specific matrix type). The allocatable attribute is only
