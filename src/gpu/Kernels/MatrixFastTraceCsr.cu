...
__global__ void MatrixFastTraceCsrKernel(const int m, const int size, int *csrRowPtr, int *csrColInd, REAL *csrVal, int nnz, REAL *result, int offset)
{
    ...
    while (i < size) {
      icol = 0;
      while ((csrColInd[csrRowPtr[i]+icol]) < i) {
        icol++;
      }
      sdata[threadIdx.x] += csrVal[csrRowPtr[i]+icol];
      i += grid_size;
    }
    __syncthreads();
